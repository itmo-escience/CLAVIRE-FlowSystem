...
namespace Easis.Common.Collections
{
    ...
    public class ListCollection<TItem> : IList<TItem>, IList
    {
        private IList<TItem> _items = new List<TItem>(); // агрегируемый список
        protected object _syncRoot = new object(); // объект синхронизации
        ...
        /// </summary>
        public ListCollection(IEnumerable<TItem> items)
        {
            AddRange(items);
        }
        ...
        {
            lock (_syncRoot)
                return new ReadOnlyCollection<TItem>(_items);
        }
        ...
        bool ICollection<TItem>.IsReadOnly
        {
            get
            {
                ...
                lock (_syncRoot)
                {
                    return ((ICollection<TItem>)_items).IsReadOnly;
                }
            }
        }
        ...
        public void AddRange(IEnumerable<TItem> range)
        {
            ...
            lock (_syncRoot)
                foreach (TItem item in range) Add(item);
        }
